...
class MazeGame:
    def __init__(self, rows, cols, goal_value, grid):
        ...
        self.explored_nodes = 0  # Initialize explored nodes counter
            
    def neighbors(self, x, y):
        for dx, dy, move in [(-1, 0, 'Left'), (0, -1, 'Up'), (1, 0, 'Right'), (0, 1, 'Down')]:  # left, up, right, down
            nx, ny = x + dx, y + dy
    ...
    def run_game(self):
        ...
        if path:
            ...
            for i in range(len(path)):
                move = path[i]
                total += move[3]  # Add the value of the current cell to the total
                print(move[2] + f"({total})", end=" -> ")
            print('End')
...
if __name__ == "__main__":
    print('Welcome to Number Maze Solver using Brute Force Algorithm!')
